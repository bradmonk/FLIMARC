function varargout = FLIMARC(varargin)
%% FLIMARC.m USAGE NOTES
%{
...
Description
-----------------------------------------------------
    FLIMARC() can be run with no arguments passed in. In this case user
...
savefileh = uicontrol('Parent', IPpanelH, 'Units', 'normalized', ...
    'Position', [0.05 0.02 0.29 0.20], 'String', 'Save Data', 'FontSize', 11,...
    'Callback', @saveDataset);

compileFilesh = uicontrol('Parent', IPpanelH, 'Units', 'normalized', ...
    'Position', [0.36 0.02 0.29 0.20], 'String', 'Compile', 'FontSize', 11,...
    'Callback', @compilefile);

loadIMGh = uicontrol('Parent', IPpanelH, 'Units', 'normalized', ...
    'Position', [0.67 0.02 0.29 0.20], 'String', 'Import Imaging Data', 'FontSize', 11,...
    'Callback', @loadIMG);
...
memes = {' ',' ',' ', ' ',' ',' ',' ', ...
         'Welcome to FLIMARC!', 'Getting things ready for you...'};
...
function loadIMG(loadIMGh, eventData)
    ...
    choice=questdlg('IMPORT GREEN OR RED CHANNEL IMAGES?','Import Menu', 'Green','Red','None','None');
    % ASK IF USER WANTS TO IMPORT RED OR GREEN CHANNEL IMAGES
    switch choice
        case 'Green'

            memocon('Select green channel image'); pause(.5);
            [ccdfileg, ccdfilegdir] = uigetfile({'*.tif*';'*.bmp';'*.jpg';'*.png'}, ...
            'load red-channel .dat file');
            ccdfiler    = [];
            ccdfilerdir = [];
        ...
        case 'Red'
            ...
            [ccdfiler, ccdfilerdir] = uigetfile({'*.tif*';'*.bmp';'*.jpg';'*.png'}, ...
            'load green-channel .bmp file');
            ccdfileg    = [];
            ccdfilegdir = [];
...
% keyboard
    ...
    if ~isempty(imSzY)
        if size(lifetime,1) > imSzY
            lifetime = lifetime(1:imSzY,:);
            intensity = intensity(1:imSzY,:);
            chi = chi(1:imSzY,:);
        elseif size(lifetime,1) < imSzY
    ...
    if ~isempty(imSzX)
        if size(lifetime,2) > imSzX
            lifetime = lifetime(:,1:imSzX);
            intensity = intensity(:,1:imSzX);
            chi = chi(:,1:imSzX);
        elseif size(lifetime,2) < imSzX
            imgG = imgG(:,1:size(lifetime,2));
            imgR = imgR(:,1:size(lifetime,2));
        end
    end

    if isempty(imSzX); imSzX = 0; end
    if isempty(imSzY); imSzY = 0; end
    ...
    boxtypeh.SelectedObject = boxtypeh3; % Set radiobutton to stamp
...
function saveDataset(hObject, eventdata)
    ...
    memocon('ALL ROI ANALYSIS SAVED  - DONE!')
